...
def download_img(csv_file_path, folder_name="cifar_10_images", img_url_col="image_link"):
  ...
  try:
    ...
    for index, row in tqdm(df.iterrows(), total=len(df), desc = "Downloading Images: "):
      ...
      try:
        ...
        #extracting img name
        if "IMG_TRANSFORMED" in img_url:
          filename = os.path.join(folder_name, id_extracter(img_url)+ ".Transformed.png")
...
def id_extracter(url):
  ...
  It's quite simple - it's just the basename of the url, without the ".jpeg".
  For example, if we have the image link : https://pilotexp.s3.ca-central-1.amazonaws.com/cifar10/IMG_TRANSFORMED/frost/batch_4_8063.png
  We'll first extract the base name with os.path.basename(url) to get "batch_4_8063.png"
  Then, we'll use the split function for ".", which will give the list ["batch_4_8063.png", "jpeg"] 
  We only need the first item for the image id
...
def csv_reader(csv_path):
  ...
  for img_id, group in tqdm(df.groupby('img_id'), desc="Processing image pairs"):
    ...
    for _, row in group.iterrows():
        ...
        ext = os.path.splitext(img2)[1]
        filename2 = os.path.basename(img2).replace(ext, ".Transformed.png")
        #filename2 = os.path.basename(img2).split(".")[0]+".Transformed.png"
...
def image_pair_prep (dataset, ds_folder='cifar_10_images'):
  ...
  for pair in dataset:
    ...
    #numpy_arrayA = preprocess_input(cv2.resize(imgA, (224, 224)))
    #numpy_arrayB = preprocess_input(cv2.resize(imgB, (224, 224)))
    numpy_arrayA = cv2.resize(imgA, (224, 224))
    numpy_arrayB = cv2.resize(imgB, (224, 224))
    ...
    X.append(numpy_arrayA)
    X.append(numpy_arrayB)
...
def compute_metrics_on_dataset(dataset, ds_folder='cifar_10_images'): #is this how you train it?
  ...
  X = image_pair_prep(dataset, ds_folder='cifar_10_images')
  y = []

  print('Before calling computeVGG')
  total_metrics = compute_VGG16_metrics(X)
  print('After calling computeVGG')
...
def display_img(index, myPairs):
  ...
  if 0 <= index < biggest:
      ...
      img1_path = os.path.join("cifar_10_images", pair_at_index['img1'])
      img2_path = os.path.join("cifar_10_images", pair_at_index['img2'])
